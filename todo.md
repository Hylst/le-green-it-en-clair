...
- [ ] veille RSS (léger) : afficher des flux RSS d'actus Green IT de sources fiables (ADEME, Arcep, GreenIT.fr, Shift Project, Next.ink…) sur `/actualites` ou une page dédiée. Cadrer : liste blanche de flux, récupération au build (site statique) ou côté client si CORS ok, carte « source externe » = titre + média + lien + date, jamais d'attribution sans origine vérifiée (règle dans `AGENTS.md`). Rappel : l'ancien `flux-rss` a été retiré le 13/09 car trop lourd à maintenir → commencer petit (3-5 flux, pas d'agrégateur complet), avec fallback si un flux tombe.
- [ ] chiffres « en direct » via API gratuites : section live (ex. sur `/chiffres` ou `/datacenters`) alimentée par des APIs sans clé ou à clé restreinte — pistes à vérifier : Impact CO₂ (ADEME), éCO2mix (RTE), Electricity Maps (free tier), ODRE / data.gouv. Contraintes : pas de backend (client-side ou génération au build), CORS, clés hors dépôt, fallback et mention « donnée indicative » si l'API ne répond pas. À sourcer avant d'afficher.
